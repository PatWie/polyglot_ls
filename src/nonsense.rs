...
///
/// Can be converted to and from [`tower_lsp::lsp_types::TextDocumentContentChangeEvent`] by
/// [`TextAdapter`].
...
/// Defines operations to convert between native text types and [`tower_lsp::lsp_types`].
/// The trait is automatically derived for any type that implements [`TextMap`].
///
/// Most operations return an [`Option`] where [`None`] signals that the
/// conversion wasn't successful.
pub trait TextAdapter {
    fn pos_to_lsp_pos(&self, pos: &Pos) -> Option<tower_lsp::lsp_types::Position>;
    fn lsp_pos_to_pos(&self, lsp_pos: &tower_lsp::lsp_types::Position) -> Option<Pos>;
    fn range_to_lsp_range(&self, range: &Range<Pos>) -> Option<tower_lsp::lsp_types::Range>;
    fn lsp_range_to_range(&self, lsp_range: &tower_lsp::lsp_types::Range) -> Option<Range<Pos>>;
    fn change_to_lsp_change(
        &self,
        change: TextChange,
    ) -> Option<tower_lsp::lsp_types::TextDocumentContentChangeEvent>;
    fn lsp_change_to_change(
        &self,
        lsp_change: tower_lsp::lsp_types::TextDocumentContentChangeEvent,
    ) -> Option<TextChange>;
}
...
impl<T: TextMap> TextAdapter for T {
    ...
    fn change_to_lsp_change(
        ...
        if let Some(range) = change.range {
            ...
            Some(tower_lsp::lsp_types::TextDocumentContentChangeEvent {
